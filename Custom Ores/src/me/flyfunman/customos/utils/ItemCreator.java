		...
		for (String item : CustomConfig.items().getKeys(false)) {
			if (clearColor(item.replace('_', ' ')).equalsIgnoreCase(clearColor(name.replace('_', ' ')))) {
				if (CustomConfig.items().contains(item + ".Enabled")
					...
					if (CustomConfig.items().contains(item + ".Enchantments.Types")
							&& !CustomConfig.items().getStringList(item + ".Enchantments.Types").isEmpty()) {
						for (String e : CustomConfig.items().getStringList(item + ".Enchantments.Types")) {
							i.addUnsafeEnchantment(
									Enchantment.getByKey(NamespacedKey.minecraft(e.toLowerCase().replace(" ", "_"))),
									CustomConfig.items().getIntegerList(item + ".Enchantments.Levels").get(CustomConfig
											.items().getStringList(item + ".Enchantments.Types").indexOf(e)));
						}
					}
